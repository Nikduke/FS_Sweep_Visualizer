...
# ---- Page config ----
st.set_page_config(page_title="FS Sweep Visualizer (Light)", layout="wide")
...
def make_line_traces(df: pd.DataFrame, cases: List[str], f_base: float, y_title: str) -> Tuple[List[go.Scatter], Optional[pd.Series]]:
    if df is None:
        return [], None
    f = df["Frequency (Hz)"]
    n = f / f_base
    ...
    for case in cases:
        ...
        traces.append(
            go.Scatter(
                x=n, y=y, customdata=cd, mode="lines", name=str(case),
                hovertemplate=("Case=%{fullData.name}<br>n=%{x:.3f}<br>f=%{customdata[0]:.1f} Hz" + f"<br>{y_title}=%{{y}}<extra></extra>")
            )
        )
    return traces, f


def apply_common_layout(fig: go.Figure, plot_height: int, y_title: str):
    fig.update_layout(
        height=plot_height,
        margin=dict(l=60, r=20, t=40, b=160),
        legend=dict(orientation="h", yanchor="top", y=-0.3, xanchor="center", x=0.5, entrywidth=180, entrywidthmode="pixels"),
    )
    fig.update_xaxes(title_text="Harmonic number n = f / f_base", tick0=1, dtick=1)
    fig.update_yaxes(title_text=y_title)


def build_single_plot(df: Optional[pd.DataFrame], cases: List[str], f_base: float, plot_height: int, y_title: str) -> Tuple[go.Figure, Optional[pd.Series]]:
    fig = go.Figure()
    traces, f_series = make_line_traces(df, cases, f_base, y_title)
    for tr in traces:
        fig.add_trace(tr)
    apply_common_layout(fig, plot_height, y_title)
    return fig, f_series


def build_x_over_r_plot(df_r: Optional[pd.DataFrame], df_x: Optional[pd.DataFrame], cases: List[str], f_base: float,
                        plot_height: int, seq_label: str) -> Tuple[go.Figure, Optional[pd.Series], int, int]:
    fig = go.Figure()
    xr_dropped = 0
    xr_total = 0
    f_series = None
    ...
    if df_r is not None and df_x is not None:
        ...
        for case in both:
            ...
            fig.add_trace(go.Scatter(x=n, y=y, customdata=cd, mode="lines", name=str(case), hovertemplate=(
...
def main():
    st.title("FS Sweep Visualizer (Light)")
    ...
    # Build plots (sequence-specific Y titles)
    r_title = "R1 (Ω)" if seq_label == "Positive" else "R0 (Ω)"
    x_title = "X1 (Ω)" if seq_label == "Positive" else "X0 (Ω)"
    fig_r, f_r = build_single_plot(df_r, filtered_cases, f_base, plot_height, r_title)
    fig_x, f_x = build_single_plot(df_x, filtered_cases, f_base, plot_height, x_title)
    fig_xr, f_xr, xr_dropped, xr_total = build_x_over_r_plot(df_r, df_x, filtered_cases, f_base, plot_height, seq_label)
